...
(async () => {
  try {
    ...
    assert.equal((await event({}, { path: '/privacy/' })).status, 204);
    assert.equal((await event({}, { path: '/ghost/settings/' })).status, 204);
    assert.equal((await event({}, {
      path: '/field-note/?secret=query-value#fragment',
      referrer: 'should-not-be-retained.example',
      account: 'forbidden-account',
      fullUrl: 'https://example.invalid/private',
      geography: 'forbidden-place'
    })).status, 204);
    assert.equal((await event({}, { path: '/' }, 'scrappykin.com')).status, 204);
